...
export function MobileTransactions() {
  ...
  const addToBatch = () => {
    ...
    if (!form.type) {
      setFormError(
        "Category has no type assigned — check your budget settings.",
      );
      return;
    }
    if (!form.amount) {
    ...
    <div style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 24px" }}>
      ...
      <div style={{ ...card, marginBottom: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 10 }}>
          ...
          />
          <button style={btn()} onClick={addToBatch}>
            + Add
          </button>
        </div>
        {formError && (
      ...
      <div style={{ ...card, padding: 16 }}>
        <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 6 }}>
          Desktop has full transaction tools
        </div>
        <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.5 }}>
          CSV import, transaction editing, filters, and deeper review are
