...
export default function CheckinPage() {
  ...
  // Compute next scheduled check-in date
  const nextScheduled = (() => {
  ...
  const daysUntilNext = Math.ceil(
    (nextScheduled.getTime() - todayObj.getTime()) / (1000 * 60 * 60 * 24)
  );
  const isScheduledToday = todayDOW === checkinDay;

  // Dispatch custom event so layout's CheckinModal opens
  ...
  return (
    <div style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 24px" }}>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              <div
                style={{ fontSize: 15, fontWeight: 700, color: C.text, marginBottom: 3 }}
              >
                {isScheduledToday
                  ? "Check-in available today"
                  : `Next check-in: ${nextScheduled.toLocaleDateString("en-US", {
                      weekday: "long",
                      month: "long",
                      day: "numeric",
                    })}`}
              ...
              <div style={{ fontSize: 13, color: C.muted }}>
                {isScheduledToday
                  ...
                  : `In ${daysUntilNext} day${daysUntilNext === 1 ? "" : "s"} — auto-opens on ${DAYS[checkinDay]}s`}
              </div>
            </div>
          </div>
      ...
      >
        <div style={{ fontSize: 13, color: C.muted }}>
          <span style={{ fontWeight: 600, color: C.text }}>Scheduled day: </span>
          {DAYS[checkinDay]}s
        </div>
          ...
          {DAYS_SHORT.map((d, i) => (
            <div
              ...
              style={{
                ...
                justifyContent: "center",
                fontSize: 11,
                fontWeight: i === checkinDay ? 700 : 500,
                background: i === checkinDay ? "#eff6ff" : C.bg,
                color: i === checkinDay ? C.accent : C.subtle,
                border: `1px solid ${i === checkinDay ? C.accent + "60" : C.borderL}`,
              }}
            >
              {d[0]}
            </div>
        ...
        <div style={{ fontSize: 12, color: C.subtle }}>
          Change in Settings → Check-in Day
        </div>
      </div>
      ...
      <div style={card}>
        ...
        {checkins.length === 0 ? (
          <div style={{ textAlign: "center", padding: "40px 0", color: C.subtle }}>
            ...
            <div
              style={{ fontSize: 14, fontWeight: 600, color: C.muted, marginBottom: 4 }}
            >
              No check-ins yet
            </div>
              ...
              >
                ...
                >
                  ...
                  >
                    {/* Date */}
                    <div
                      style={{ fontSize: 13, fontWeight: 700, color: C.text, flexShrink: 0 }}
                    >
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      ...
                      >
                        Spent {fmt(ci.weekSpend)}
                      </span>

                      {ci.weekDebt && parseFloat(ci.weekDebt) > 0 && (
                        <span
                          style={{
                            fontSize: 12,
                            background: "#dcfce7",
                      ...
                      {ci.topCatName && (
                        ...
                        >
                          Top: {ci.topCatName}
                          {ci.topCatAmount && parseFloat(ci.topCatAmount) > 0
                            ? ` · ${fmt(ci.topCatAmount)}`
                            : ""}
                        </span>
                      )}
