...
const CT = ({ active, payload }: { active?: boolean; payload?: Array<{ name: string; value: number; color?: string }> }) => {
...
export function MobileOverview() {
  const { months, filterMonth, setFilterMonth, stats, dueSoon, donutData } = useOverviewData();
  ...
  return (
    <div style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 24px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
        ...
        <select
          style={{ background: C.surf, border: `1px solid ${C.border}`, color: C.text, padding: "7px 12px", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}
          value={filterMonth}
      ...
      <div className="grid grid-cols-2 min-[768px]:grid-cols-3 min-[1280px]:grid-cols-5 gap-3 mb-3.5">
        {[
          ...
          <div key={k.label} style={card}>
            ...
            <div style={{ fontSize: 18, fontWeight: 800, letterSpacing: -0.5, color: k.pos === null ? C.text : k.pos ? C.green : C.red }}>{fmt(k.val)}</div>
          </div>
        ))}
      </div>
      ...
      {dueSoon.length > 0 && (
        <div style={{ ...card, marginBottom: 14, padding: "14px 16px" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 0.6, marginBottom: 10 }}>Bills Due Soon</div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {dueSoon.map(b => {
              ...
              return (
                <div key={b.name} style={{ background: bg, border: `1px solid ${color}30`, borderRadius: 8, padding: "8px 10px", minWidth: 120 }}>
      ...
      <div style={{ ...card, marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 0.6, marginBottom: 12 }}>Money Out by Category</div>
      ...
      <div style={{ ...card, padding: 16 }}>
        ...
        <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.5 }}>Open Kova on desktop to review category tables, debt payoff details, trends, calendar, and check-ins.</div>
      </div>
    </div>
  );
}
