...
export default function DesktopCalendar() {
  ...
  const calData = useMemo(() => {
    ...
    transactions.filter(t => t.date.startsWith(calMonth)).forEach(t => {
      ...
      if (t.type !== "INCOME") { txnsByDay[d].count++; txnsByDay[d].amount += parseFloat(t.amount); }
    });
      ...
      <div style={card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4 }}>
          ...
          {Array.from({ length: calData.daysInMonth }, (_, i) => i + 1).map(day => {
            ...
            const hasTxns = calData.txnsByDay[day as keyof typeof calData.txnsByDay];
            const bills = (calData.billsByDay as Record<number, { name: string; amount: number }[]>)[day] || [];
            return (
              <div key={day} style={{ minHeight: 80, background: isToday ? "#eff6ff" : C.bg, borderRadius: 8, padding: "6px 8px", border: `1px solid ${isToday ? C.accent : C.borderL}`, position: "relative" }}>
