...
function BudgetTargetsTab() {
  ...
  // Update rows when budgets change (e.g. after adding a category)
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>
            Set monthly budget amounts for each category. Bills can also have a due day.
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {saved && <SavedBadge />}
          <button style={btn()} onClick={handleSave} disabled={saving}>
            {saving ? "Saving…" : "Save Budget Targets"}
          </button>
        </div>
      </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    ...
                    {type === "BILLS" && <th style={th}>Due Day</th>}
                  </tr>
                </thead>
                <tbody>
                  {typeRows.map((row) => (
                      ...
                      <td style={td}>
                        <div style={{ position: "relative", maxWidth: 160 }}>
                          ...
                          <input
                            ...
                            min={0}
                            step={1}
                            style={{ ...inp, paddingLeft: 22, maxWidth: 160 }}
                            value={row.budgetAmount}
                            onChange={(e) =>
                              updateRow(row.name, "budgetAmount", e.target.value)
                            }
                          />
                        </div>
                      </td>
        ...
        {rows.length === 0 && (
          ...
          >
            ...
            <div style={{ fontSize: 13 }}>
              Add categories in the Categories tab to set budget targets.
            </div>
          </div>
        )}
      </div>
...
function CategoriesTab() {
  ...
  const handleAdd = async () => {
    ...
    if (!newAmount || isNaN(parseFloat(newAmount))) {
      setAddError("Enter a valid budget amount.");
      return;
    }
    const exists = budgets.some(
      (b) => b.name.toLowerCase() === trimmed.toLowerCase() && b.type === newType
    );
    ...
    <div>
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: C.muted }}>
          Add new categories. To remove a category, set its budget amount to 0 in Budget Targets.
        </div>
      </div>
      ...
      <div style={{ ...card, marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
          <div style={{ flex: "2 1 160px", minWidth: 140 }}>
            <label
              style={{ fontSize: 11, fontWeight: 600, color: C.muted, display: "block", marginBottom: 5 }}
            >
              Name
            </label>
            <input
          ...
          <div style={{ flexShrink: 0, paddingBottom: 1 }}>
            <button style={btn()} onClick={handleAdd} disabled={adding}>
              {adding ? "Adding…" : "Add Category"}
            </button>
          </div>
        </div>
        {addError && (
          <div style={{ fontSize: 12.5, color: C.red, marginTop: 8 }}>{addError}</div>
        )}
      </div>
          ...
          return (
            <div key={type} style={card}>
                ...
                <tbody>
                  {cats.map((b) => (
                    <tr key={b.name}>
                      ...
                      <td style={td}>
                        ${Number(b.budgetAmount).toLocaleString("en-US", {
              ...
              <div style={{ fontSize: 12, color: C.subtle, marginTop: 10 }}>
                To remove a category, set its budget to $0 in Budget Targets.
              </div>
            </div>
          );
        })}
...
function DebtBalancesTab() {
  ...
  const debtBudgets = useMemo(
    () => budgets.filter((b) => b.type === "DEBT PAYMENT"),
    [budgets]
  );
  ...
  // Sync rows when debt budgets change
  ...
  const updateBalance = (name: string, value: string) => {
    setRows((prev) =>
      prev.map((r) => (r.name === name ? { ...r, startingBalance: value } : r))
    );
    setSaved(false);
  };
      ...
      const results = await Promise.allSettled(
        rows.map((r) =>
          upsertBudget({
            ...
            dueDay: r.dueDay,
            startingBalance: r.startingBalance || "0",
          })
        )
      );
      const failures = results.filter(r => r.status === "rejected").length;
    ...
    <div>
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>
            Set the starting balance for each debt account to track payoff progress.
          </div>
        </div>
        {rows.length > 0 && (
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      {rows.length === 0 ? (
        ...
        >
          ...
          <div style={{ fontSize: 13 }}>
            Add a &ldquo;Debt Payment&rdquo; category in the Categories tab to track balances here.
          </div>
        </div>
      ) : (
        <div style={card}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                <th style={th}>Account / Category</th>
                <th style={th}>Monthly Payment</th>
                <th style={th}>Starting Balance</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row.name}>
                  <td style={{ ...td, fontWeight: 600 }}>{row.name}</td>
                  <td style={{ ...td, color: C.muted }}>
                    ${Number(row.budgetAmount).toLocaleString("en-US", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                    /mo
                  ...
                  <td style={td}>
                    <div style={{ position: "relative", maxWidth: 180 }}>
                      ...
                      <input
                        ...
                        min={0}
                        step={1}
                        style={{ ...inp, paddingLeft: 22, maxWidth: 180 }}
                        value={row.startingBalance}
                        onChange={(e) => updateBalance(row.name, e.target.value)}
                      />
                    </div>
                  </td>
                </tr>
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: C.muted }}>
          Choose which day of the week Kova automatically opens the check-in modal.
        </div>
      </div>
      ...
      <div style={{ ...card, maxWidth: 520 }}>
          ...
          {DAYS_SHORT.map((d, i) => (
            <button
              ...
              style={{
                padding: "10px 0",
                borderRadius: 8,
                border: `1.5px solid ${i === selected ? C.accent : C.borderL}`,
                cursor: "pointer",
                fontFamily: "inherit",
                fontSize: 12.5,
                fontWeight: i === selected ? 700 : 500,
                transition: "all 0.12s",
                background: i === selected ? "#eff6ff" : C.bg,
        ...
        >
          ...
          {selected !== settings.checkinDay && (
            <div style={{ fontSize: 12, color: C.amber, marginTop: 4 }}>
              Unsaved change — was {DAYS_FULL[settings.checkinDay]}.
            </div>
          )}
        </div>
        ...
        <div style={{ marginTop: 16, fontSize: 12.5, color: C.subtle, lineHeight: 1.6 }}>
          On {DAYS_FULL[selected]}s, the check-in modal opens automatically when you visit
          the dashboard (once per week). You can also start a check-in at any time from the
          header or the Check-in page.
        </div>
      </div>
    </div>
  );
